...
HeatEquation::HeatEquation(std::shared_ptr<TypeXVector> vec_X,
  ...
  S_ = std::make_shared<TypeS>(P_Y_, B_, BT_, G0_);
}
...
NewMethodHeatEquation::NewMethodHeatEquation(std::shared_ptr<TypeXVector> vec_X,
                                             std::shared_ptr<TypeYVector> vec_Y,
...
NewMethodHeatEquation::NewMethodHeatEquation(std::shared_ptr<TypeXVector> vec_X,
                                             std::shared_ptr<TypeYVector> vec_Y,
                                             const HeatEquationOptions &opts)
    : NewMethodHeatEquation(vec_X, vec_Y, nullptr, nullptr, false, opts) {}

NewMethodHeatEquation::NewMethodHeatEquation(const TypeXDelta &X_delta,
                                             const TypeYDelta &Y_delta,
                                             const HeatEquationOptions &opts)
    : NewMethodHeatEquation(std::make_shared<TypeXVector>(
                                X_delta.template DeepCopy<TypeXVector>()),
                            std::make_shared<TypeYVector>(
                                Y_delta.template DeepCopy<TypeYVector>()),
                            nullptr, nullptr, true, opts) {}

NewMethodHeatEquation::NewMethodHeatEquation(
    const DoubleTreeView<ThreePointWaveletFn, HierarchicalBasisFn> &X_delta,
    const HeatEquationOptions &opts)
    : NewMethodHeatEquation(X_delta, GenerateYDelta<DoubleTreeView>(X_delta),
                            opts) {}

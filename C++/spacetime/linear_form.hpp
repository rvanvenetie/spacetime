...
class LinearForm {
 public:
  LinearForm(Time::LinearForm<TimeBasis> time_linform, const SpaceF &space_f)
  ...
  void Apply(datastructures::DoubleTreeVector<TimeBasis, SpaceBasis> *vec) {
    ...
    for (auto phi : project_0->Bfs())
      phi->node()->template set_data<double>(
          const_cast<double *>(&phi->value()));
    for (auto phi : project_1->Bfs())
      phi->node()->template set_data<double>(
          const_cast<double *>(&phi->value()));

    for (auto dblnode : vec->Bfs())
      dblnode->set_value(
          *std::get<0>(dblnode->nodes())->template data<double>() *
          *std::get<1>(dblnode->nodes())->template data<double>());

    for (auto phi : project_0->Bfs()) phi->node()->reset_data();
    for (auto phi : project_1->Bfs()) phi->node()->reset_data();
  }

 protected:
  Time::LinearForm<TimeBasis> time_linform_;
  const SpaceF &space_f_;
};

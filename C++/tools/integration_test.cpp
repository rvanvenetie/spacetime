...
TEST(Integration1D, ProductOfMonomials) {
  time::elem_tree.UniformRefine(4);
  ...
  static_for<10>([&T](auto degree) {
    ...
    for (size_t n = 0; n < degree; n++) {
      ...
      EXPECT_NEAR(IntegrationRule<1, degree>::Integrate(f, *root),
                  (pow(b, n) - pow(a, n)) / (n + 1), 1e-10);
    }
  });
}
...
TEST(Integration2D, ProductOfMonomials) {
  ...
  static_for<10>([&T](auto degree) {
    for (auto elem : T.elem_tree.Bfs()) {
      ...
      EXPECT_NEAR(IntegrationRule<2, degree>::Integrate(f, *elem), elem->area(),
                  1e-10);
    }
  });
  ...
  static_for<10>([&T](auto degree) {
    ...
    for (size_t n = 0; n < degree; n++) {
      for (size_t m = 0; m + n <= degree; m++) {
        ...
        EXPECT_NEAR(
            IntegrationRule<2, degree>::Integrate(f, *root),

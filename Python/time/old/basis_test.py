...
def test_haar_ss_nonzero_in_nbrhood():
    ...
    # Check level 0
    assert basis.scaling_indices_nonzero_in_nbrhood(
        0, 1 - eps).indices == {(0, 0)}
    ...
    assert basis.scaling_indices_nonzero_in_nbrhood(
        2,
        F(1, 4) - eps).indices == {(2, 0)}
    assert basis.scaling_indices_nonzero_in_nbrhood(2, F(
        1, 4)).indices == {(2, 0), (2, 1)}
    assert basis.scaling_indices_nonzero_in_nbrhood(2, F(
        1, 2)).indices == {(2, 1), (2, 2)}
    assert basis.scaling_indices_nonzero_in_nbrhood(2, F(
    ...
    assert basis.scaling_indices_nonzero_in_nbrhood(40, F(
        13, 2**40)).indices == {(40, 12), (40, 13)}
    assert basis.scaling_indices_nonzero_in_nbrhood(40, F(
        876, 2**40)).indices == {(40, 875), (40, 876)}

    # Check 500 points for levels 0..123.
    for l in range(123):
        for pt in range(max(1, 2**l - 500), 2**l):
            assert basis.scaling_indices_nonzero_in_nbrhood(
                l, pt * F(1, 2**l)).indices == {(l, pt - 1), (l, pt)}
...
def test_orthonormal_ss_nonzero_in_nbrhood():
    ...
    for l in range(14):
        for n in range(2**(l + 1)):
            ...
            assert labda in basis.scaling_indices_nonzero_in_nbrhood(
                l, support.a)
            assert labda in basis.scaling_indices_nonzero_in_nbrhood(
                l, support.mid)
            assert labda in basis.scaling_indices_nonzero_in_nbrhood(
            ...
            assert len(
                basis.scaling_indices_nonzero_in_nbrhood(l, support.mid)) == 2

            if n > 1 and n < 2**(l + 1) - 2:
                assert len(
                    basis.scaling_indices_nonzero_in_nbrhood(
                        l, support.a)) == len(
                            basis.scaling_indices_nonzero_in_nbrhood(
                                l, support.b)) == 4
...
def test_orthonormal_scaling_mass():
    ...
    for l in range(1, 5):
        ...
        for _ in range(100):
            d = SparseVector(indices, np.random.rand(2 * 2**l))
            res = mass.matvec(d, indices, indices)
            assert np.allclose(d.asarray(), 2.0**l * res.asarray())
    ...
    for basis in [
        ...
        for l in range(1, ml + 1):
            ...
            for i, mu in enumerate(Lambda_l):
                ...
                vec = SparseVector(Lambda_l, eye[i, :])
...
def test_basis_PQ_domain_range():
    ...
    for basis in [
        ...
        for l in range(1, ml + 1):
            print('test PQ domain/range on level {} for {}'.format(
                l, basis.__class__.__name__))
            Delta_l = basis.scaling_indices_on_level(l)
            Lambda_l = basis.wavelet_indices_on_level(l)

            assert basis.P.range(Delta_l) == basis.scaling_indices_on_level(l +
                                                                            1)
            assert basis.P.domain(Delta_l) == basis.scaling_indices_on_level(
                l - 1)
...
def test_singlescale_quadrature():
    ...
    for basis_in, basis_out, operator, deriv in [
        ...
        print('basis_in = {}\tbasis_out={}'.format(
        ...
        for l in range(1, ml + 1):
            ...
            for i, labda in enumerate(Delta_l_in):
                ...
                unit_vec = SparseVector(Delta_l_in, eye[i, :])
                out = operator.matvec(unit_vec, Delta_l_in, Delta_l_out)
                for mu in Delta_l_out:
                    supp = phi_supp.intersection(
                        support_to_interval(basis_out.scaling_support(mu)))
                    true_val = 0.0
                    if supp:
                        true_val = quad(
...
def print_3point_functions():
    ...
    for basis in [
        ...
        for l in range(basis.indices.maximum_level + 1):
            ...
            plt.title("Scaling functions on level {} for {}".format(
                l, basis.__class__.__name__))
            plt.legend()
            plt.show()
